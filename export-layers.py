...
def format_filename(img, layer, include_this_filename):

  imgname = img.name.decode('utf-8')
  layername = layer.name.decode('utf-8')
  regex = re.compile("[^-\w]", re.UNICODE)
  project_name, project_extension = os.path.splitext(imgname)

  if include_this_filename:
    filename = regex.sub('_', project_name) + '-' + regex.sub('_', layername) + '.png'
  else:
...
def export_layers(img, drw, path, include_this_filename=True, only_visible=True, flatten=False, remove_offsets=False, crop=False):
  dupe = img.duplicate()
  layers = get_layers(dupe.layers, only_visible)

  for layer in layers:
    layer.visible = 1
    filename = format_filename(img, layer, include_this_filename)
...
register(
  ...
  params=[
    ...
    (PF_BOOL, "include_this_filename", "Include this filename?", True),
